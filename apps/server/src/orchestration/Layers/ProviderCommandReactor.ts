...
const make = Effect.gen(function* () {
  ...
  const buildSendTurnRequestForThread = Effect.fnUntraced(function* (input: {
    ...
    // Upstream also writes `threadModelSelections` here. This fork deliberately
    // does not: the deferred model-change recycle decides whether a pending
    // recycle is still needed by comparing it against this map (see the
    // Equal.equals check in the turn-completed flush), so recording the
    // selection eagerly at turn start makes every pending recycle look already
    // applied and it never flushes. Keep upstream's `pendingTurnStart` flag.
