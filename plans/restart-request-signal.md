...
3. **UI:** **rung 1 + rung 2** together — the sidebar pill/sibling tint *and* the sibling "hold if you can" banner. Both are cheap once the flag exists, and the banner is what actually delivers the "steer away from the other chats" goal. Neither blocks send, so false positives are harmless.
